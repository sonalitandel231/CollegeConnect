...
<script>
  ...
  xhr.open("POST", "check_like_class.php", true);
...
function updateLikeCount(postId) {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "updatelikes2.php", true);
  xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
...
document.addEventListener('click', function(event) {
  if (event.target.classList.contains('post-comment-btn')) {
    ...
    if (comment !== '') {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "add_comment2.php", true);
      xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          // Handle the response
...
function fetchComments(postId) {
  ...
  xhr.open('GET', `fetch_comments2.php?Postid=${postId}`, true);
  xhr.onload = function() {
    if (xhr.status === 200) {
